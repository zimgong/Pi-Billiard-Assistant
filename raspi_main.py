...
def grab_frame_display(run_flag, frame_queue, line_queue, table_queue, dev):
	...
	while run_flag.value:
		...
		if delta_time_ms > 10 and frame_queue.qsize() < 3: # If past time and queue is not full, send to queue
...
def process_stick_1(run_flag, frame_queue, table_queue, line_queue, dev):
	table = []
	while run_flag.value:
...
def process_stick_2(run_flag, frame_queue, table_queue, line_queue, dev):
	table = []
	while run_flag.value:
		if not frame_queue.empty():
			frame_hsv = frame_queue.get() # Get frame from queue
...
def process_physics(run_flag, frame_queue, table_queue, line_queue, dev):
	table = []
	while run_flag.value:
		if not frame_queue.empty():
			frame_hsv = frame_queue.get() # Get frame from queue
...
if __name__ == '__main__':
    ...
    p1 = Process(target=process_stick_1, args=(run_flag, frame_queue, table_queue, line_queue, dev))
    p2 = Process(target=process_stick_2, args=(run_flag, frame_queue, table_queue, line_queue, dev))
    p3 = Process(target=process_physics, args=(run_flag, frame_queue, table_queue, line_queue, dev))
    # start processes
    p0.start()
    p1.start()
    p2.start()
